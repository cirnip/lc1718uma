...
% A funÃ§Ã£o teorema/1 Ã© um predicado que recebe uma lista de lista, verifica se o
% converte/1 da negaÃ§Ã£o de "fi" Ã© refutÃ¡vel e se for refutÃ¡vel ele
% devolve que "fi" Ã© teorema no sistema de RP, caso contrario diz que
% nÃ£o Ã© teorema em RP.

/* Exemplo de input: teorema([[(p imp (q imp r)) imp ((p imp q) imp (p imp r))]]).
 * O input acima Ã© um teorema. */

/* Exemplo de input: teorema([[(neg p imp q) imp (p imp q)]]).
 * O input acima NÃƒO Ã© um teorema. */
